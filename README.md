...
为了尽量兼容当前字幕翻译场景，字幕文件翻译现在默认采用“官方批量翻译”模式：

- 服务端会直接把每条字幕文本作为一个独立翻译项，通过 `@cf/meta/m2m100-1.2b` 的 `requests` 批量接口一次提交
- 每个返回项与原字幕段天然一一对应，不再依赖额外的行号前缀或提示词回填
- 这样可以在保持字幕时间轴不变的前提下，用一次官方支持的批量翻译请求完成字幕翻译
