...
为了尽量兼容当前字幕翻译场景，字幕文件翻译现在默认采用“整份索引翻译”模式：

- 服务端会先把整份字幕转成“序号 + 文本”的索引行，再一次性交给翻译模型
- 当前索引格式使用更直接的 `1 文本`、`2 文本` 这种纯数字前缀，保持输入尽量接近普通翻译文本
- 当前做法不额外拼接提示词或说明文本，而是直接提交“`序号 + 待翻译文字`”这种整份索引内容
- 翻译完成后再根据每行前面的序号把中文回填到原始字幕时间轴
- 这样可以在保持字幕时间轴不变的前提下，用一次整份翻译请求完成字幕翻译

这能提升 Free 账号下的成功率，但代价是：

- 如果模型改写、丢失或打乱索引行，系统会拒绝回填，避免错位字幕
