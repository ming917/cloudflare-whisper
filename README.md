...
为了尽量兼容当前字幕翻译场景，字幕文件翻译现在默认采用“官方数组批量翻译 + 前端轮询”模式：

- 服务端会把每条字幕文本作为一个数组项，通过 `@cf/meta/m2m100-1.2b` 的 Batch API 提交
- Worker 只负责提交批量任务并返回 `request_id`
- 前端会单独轮询 `/subtitle/status`，拿到结果后再按返回项索引回填到原字幕段
- 这样可以避免把字幕拼成一大段文本，也能保持字幕段与翻译结果的一一对应

这能提升 Free 账号下的成功率，但代价是：

- 超长字幕仍可能因为 Free 账号的提交体积、平台队列或等待时间限制而失败
